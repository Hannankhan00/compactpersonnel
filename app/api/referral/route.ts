...
export async function POST(request: Request) {
    try {
        ...
        const data = {
            referrerFirstName: normalize(payload.referrerFirstName),
            referrerSurname: normalize(payload.referrerSurname),
            referrerEmail: normalize(payload.referrerEmail),
            referrerPhone: normalize(payload.referrerPhone),
            referrerPosition: normalize(payload.referrerPosition),
            personInitials: normalize(payload.personInitials),
            personAge: normalize(payload.personAge),
            personGender: normalize(payload.personGender),
            diagnosis: normalize(payload.diagnosis),
            currentAccommodation: normalize(payload.currentAccommodation),
            sectioned: normalize(payload.sectioned),
            reasonForPlacement: normalize(payload.reasonForPlacement),
            disagreements: normalize(payload.disagreements),
            timescale: normalize(payload.timescale),
            accommodationType: normalize(payload.accommodationType),
            fundingPosition: normalize(payload.fundingPosition),
            authority: normalize(payload.authority),
            dolCopStatus: normalize(payload.dolCopStatus),
            capacityAssessments: normalize(payload.capacityAssessments),
            levelOfSupport: normalize(payload.levelOfSupport),
        };

        const requiredValues = Object.values(data);
        ...
        const smtpUser = process.env.SMTP_USER!;
        const recipientEmail = process.env.REFERRAL_FORM_TO ?? 'info@compactpersonnel.co.uk';
        const fromEmail = process.env.REFERRAL_FORM_FROM ?? process.env.JOB_APPLICATION_FROM ?? smtpUser;
        ...
        const subject = `Referral Submission from ${data.referrerFirstName} ${data.referrerSurname}`;
        const emailBody = [
            'NEW REFERRAL SUBMISSION',
            '',
            '=== REFERRER DETAILS ===',
            `Name: ${data.referrerFirstName} ${data.referrerSurname}`,
            `Email: ${data.referrerEmail}`,
            `Phone: ${data.referrerPhone}`,
            `Position: ${data.referrerPosition}`,
            '',
            '=== PERSON SUPPORTED ===',
            `Initials: ${data.personInitials}`,
            `Age: ${data.personAge}`,
            `Gender: ${data.personGender}`,
            `Diagnosis/Behaviours: ${data.diagnosis}`,
            '',
            '=== CURRENT POSITION ===',
            `Current Accommodation: ${data.currentAccommodation}`,
            `Sectioned: ${data.sectioned}`,
            `Reason for Placement: ${data.reasonForPlacement}`,
            `Disagreements with Provider: ${data.disagreements}`,
            `Timescale: ${data.timescale}`,
            '',
            '=== ACCOMMODATION REQUIRED ===',
            `Type Required: ${data.accommodationType}`,
            `Funding Position: ${data.fundingPosition}`,
            `Authority/Funding Provider: ${data.authority}`,
            `DOL/COP Status: ${data.dolCopStatus}`,
            `Capacity Assessments: ${data.capacityAssessments}`,
            `Level of Support: ${data.levelOfSupport}`,
        ].join('\n');
        ...
        await transporter.sendMail({
            ...
            subject: 'We received your referral message - Compact Personnel',
            text: [
                `Hello ${data.referrerFirstName} ${data.referrerSurname},`,
                '',
            ...
            html: buildAutoResponseHtml({
                companyName,
                fullName: `${data.referrerFirstName} ${data.referrerSurname}`,
                websiteUrl,
                logoUrl,
                recipientEmail,
            }),
        ...
        return NextResponse.json({
            message: 'Referral submitted successfully. Our team will review it and contact you soon.',
        });
    } catch (error) {
        console.error('Failed to send referral form email', error);

        return NextResponse.json(
            { error: 'We could not send your referral right now. Please try again later.' },
            { status: 500 }
        );
    }
}